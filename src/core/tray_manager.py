...
from ..utils.runtime import should_enable_auto_update

class TrayManager:
    """System tray manager for Tranfastic"""
    
    def __init__(self, on_settings: Callable, on_restart: Callable, on_exit: Callable, on_check_updates: Optional[Callable] = None):
    ...
    def _create_icon(self):
        ...
        try:
            ...
            # Add check for updates menu if callback is provided and auto-update is enabled
            if self.on_check_updates and should_enable_auto_update():
                menu_items.append(pystray.MenuItem("Check for Updates", self._on_check_updates_click))
    ...
    def _on_check_updates_click(self, icon, item):
        """Handle check for updates menu click"""
        try:
            if self.on_check_updates:
                self.on_check_updates()
        except Exception as e:
            self.logger.error(f"Check updates click error: {e}")
